    ...
    <div className="w-full max-w-lg bg-white dark:bg-black rounded-xl shadow-md flex flex-col items-center justify-center">
          ...
          {signer && Object.entries(options || {}).filter(([key, value]) => value && items.findIndex(item => item.name === key) > -1).length >= items.length && (
            <div className="flex justify-end space-x-2 mb-2">
              ...
              <button
                disabled={minting}
                onClick={() => mint()}
                className="btn btn-default btn-rounded bg-purple-500 hover:bg-purple-600 flex items-center text-white space-x-1.5"
              >
                {minting && (
      ...
      {mintResponse && (
        <div className="mx-2 sm:mx-4">
          <Alert
            color={`${mintResponse.status === 'failed' ? 'bg-red-400 dark:bg-red-500' : mintResponse.status === 'success' ? 'bg-green-400 dark:bg-green-500' : 'bg-purple-400 dark:bg-purple-500'} text-white mb-4 sm:mb-6`}
