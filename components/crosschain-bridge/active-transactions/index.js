        ...
        title={<div className="flex items-center">
          ...
          {t && (
            <div className="flex items-center space-x-1">
              <span className="cursor-pointer text-purple-400 dark:text-white text-sm font-medium">
      ...
      >
        <div className="flex flex-col items-center space-y-2 mt-2">
          <div className="flex items-center justify-center space-x-3">
            ...
            />
            <TiArrowRight size={24} className="text-purple-600 dark:text-white" />
            <Img
              src={toChain?.image}
              alt=""
              className="w-7 h-7 rounded-full"
          ...
          <div className="flex flex-col items-center">
            {t?.status === NxtpSdkEvents.SenderTransactionPrepared ?
              <div className="flex items-center justify-center space-x-2">
                <span className="text-purple-600 dark:text-white font-medium">Waiting for Router</span>
                <Bars color={theme === 'dark' ? 'white' : '#7c3aed'} width="16" height="16" />
              </div>
              :
              t?.status === NxtpSdkEvents.ReceiverTransactionPrepared ?
                <HeadShake duration={1500} forever>
                  <button
                    type="button"
                    onClick={() => onClick(t)}
                    className="bg-purple-600 hover:bg-purple-500 dark:bg-purple-600 dark:hover:bg-purple-700 rounded-xl flex items-center justify-center text-gray-100 hover:text-white text-xs space-x-1 mb-1 py-1.5 pl-2.5 pr-1.5"
                  >
                ...
                t?.status === NxtpSdkEvents.ReceiverPrepareSigned ?
                  <div className="flex items-center justify-center space-x-2">
                    <span className="text-purple-600 dark:text-white font-medium">Waiting for Relayer</span>
                    <Oval color={theme === 'dark' ? 'white' : '#7c3aed'} width="16" height="16" />
                  </div>
                  :
                  null
            }
