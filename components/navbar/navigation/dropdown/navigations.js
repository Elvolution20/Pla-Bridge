  ...
  return (
    <div className="flex flex-wrap">
      {navigations.filter(item => item?.path).map((item, i) => {
        const className = `dropdown-item w-full bg-transparent flex items-center uppercase ${pathname === item.path ? 'text-black dark:text-white font-bold' : 'text-purple-600 dark:text-white font-medium'} p-3`
